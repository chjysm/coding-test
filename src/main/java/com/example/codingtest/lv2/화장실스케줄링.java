...
public class 화장실스케줄링 {
    ...
    public static class Person{
        ...
        public int goTime() throws InterruptedException {
            if(remainTime > 0){
                --remainTime;
            }
            return remainTime;
    ...
    public static class Toilet implements Runnable {
        ...
        public void run() {
            ...
            // 다음 순번 폴링 및 구동
            Toilet poll = currentToilets.poll();
            try {
    ...
    public static class Scheduler{
        private final int m; // 사람 수
        private final int n; // 화장실 수
        private final BlockingQueue<Toilet> currentToilets;
        private final ExecutorService executorService;

        Scheduler( int m, int n ){
            this.m = m;
            this.n = n;
            this.currentToilets = new ArrayBlockingQueue<>(n);
            this.executorService = Executors.newFixedThreadPool(m);
        }
        ...
        public void start() {
            try {
                ...
                for(int i = 1; i<=m ; i++){
                    ...
                    Toilet toilet = new Toilet( new Person(i, randomNum), currentToilets, remainToiletList, executorService);
    ...
    public static void main(String[] args){
        Scheduler scheduler = new Scheduler(10, 10);
        scheduler.start();
    }
}
